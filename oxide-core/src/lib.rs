//! oxide is a Rust library for building LLM-powered applications that focuses on ergonomics and modularity.
//!
//! # Table of contents
...
//! # Simple example:
//! ```
//! use oxide::{completion::Prompt, providers::openai};
//!
//! #[tokio::main]
//! async fn main() {
...
//! oxide provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! oxide also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! oxide provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom oxidehitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! oxide natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! oxide currently supports the following vector store integrations via companion crates:
//! - `oxide-mongodb`: Vector store implementation for MongoDB
//! - `oxide-lancedb`: Vector store implementation for LanceDB
//! - `oxide-neo4j`: Vector store implementation for Neo4j
//! - `oxide-qdrant`: Vector store implementation for Qdrant
//!
