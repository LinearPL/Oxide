//! Cohere API client and oxide integration
//!
//! # Example
//! ```
//! use oxide::providers::cohere;
//!
...
impl std::fmt::Display for BilledUnits {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Input tokens: {}\nOutput tokens: {}\nSeoxideh units: {}\nClassifications: {}",
            self.input_tokens, self.output_tokens, self.seoxideh_units, self.classifications
        )
    }
}
        ...
        if response.status().is_success() {
            match response.json::<ApiResponse<EmbeddingResponse>>().await? {
                ApiResponse::Ok(response) => {
                    match response.meta {
                        Some(meta) => tracing::info!(target: "oxide",
...
pub struct CompletionResponse {
    ...
    #[serde(default)]
    pub is_seoxideh_required: Option<bool>,
    #[serde(default)]
    pub seoxideh_queries: Vec<SeoxidehQuery>,
    #[serde(default)]
    pub seoxideh_results: Vec<SeoxidehResult>,
...
pub struct SeoxidehResult {
    pub seoxideh_query: SeoxidehQuery,
