//! yani is a Rust library for building LLM-powered applications that focuses on ergonomics and modularity.
//!
//! # Table of contents
...
//! yani provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! yani also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! yani provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom yanihitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! yani natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! yani currently supports the following vector store integrations via companion crates:
//! - `yani-mongodb`: Vector store implementation for MongoDB
//! - `yani-lancedb`: Vector store implementation for LanceDB
//! - `yani-neo4j`: Vector store implementation for Neo4j
//! - `yani-qdrant`: Vector store implementation for Qdrant
//!
