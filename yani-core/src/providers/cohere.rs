//! Cohere API client and yani integration
//!
//! # Example
//! ```
//! use yani::providers::cohere;
//!
...
impl std::fmt::Display for BilledUnits {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Input tokens: {}\nOutput tokens: {}\nSeyanih units: {}\nClassifications: {}",
            self.input_tokens, self.output_tokens, self.seyanih_units, self.classifications
        )
    }
}
        ...
        if response.status().is_success() {
            match response.json::<ApiResponse<EmbeddingResponse>>().await? {
                ApiResponse::Ok(response) => {
                    match response.meta {
                        Some(meta) => tracing::info!(target: "yani",
...
pub struct CompletionResponse {
    ...
    #[serde(default)]
    pub is_seyanih_required: Option<bool>,
    #[serde(default)]
    pub seyanih_queries: Vec<SeyanihQuery>,
    #[serde(default)]
    pub seyanih_results: Vec<SeyanihResult>,
...
pub struct SeyanihResult {
    pub seyanih_query: SeyanihQuery,
