...
impl completion::CompletionModel for CompletionModel {
    ...
    async fn completion(
        ...
        match response.usage_metadata {
            Some(ref usage) => tracing::info!(target: "yani",
            "Gemini completion token usage: {}",
            usage
            ),
            None => tracing::info!(target: "yani",
...
pub mod gemini_api_types {
    ...
    pub struct ContentCandidate {
        ...
        /// These are passages that are "recited" from copyyanihted material in the foundational LLM's training data.
    ...
    /// ### yani Note:
    /// Can be used to cosntruct a typesafe `additional_params` in yani::[AgentBuilder](crate::agent::AgentBuilder).
    #[derive(Debug, Deserialize, Serialize)]
    #[serde(rename_all = "camelCase")]
